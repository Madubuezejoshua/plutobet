...
| **An unattended real settlement** | `WAITING_ON_REAL_EVENT` | See §3 — not achieved, and why |
...
**Not proven — stated plainly:** no new bet was placed and settled unattended
during this session. The local dev server repeatedly stopped responding, and the
cause is worth recording because it is not only a local annoyance:
...
> query against a cold Neon instance blocks every other request. That is what
> wedged the dev server here, with the scheduler stopped.
>
> This should be treated as a launch-blocking review item for Railway
> specifically, not as a local quirk.

**To finish this observation**, with a fixture that kicks off soon:

```bash
npm run dev:all                                    # app + scheduler
npx tsx scripts/qa-http-register.ts http://127.0.0.1:3000
npx tsx scripts/qa-credit.ts <userId> 20000        # ₦200, gated and audited
npx tsx scripts/qa-http-bet.ts <email> <password> http://127.0.0.1:3000
npm run settle:watch -- <betId> --follow           # READ ONLY, cannot settle
```

The watcher runs inside a `READ ONLY` transaction, so the database itself refuses
any write it might attempt. Settlement must come from the cron or it does not
count. **Do not** use `scripts/qa-settle-run.ts` or `qa-settle-one.ts` to close
this item — those invoke settlement manually and prove only that a human can.
...
See the completion report for the exact command output. Summary: typecheck clean,
lint clean, full suite green with 1 deliberate skip (the opt-in live provider
contract), production build clean, 26/26 migrations applied to a clean database,
ledger balanced, secret scan clean, `git diff --check` clean.
